...
ROBOOP_ROBOT_MEX_FUNC_START
    ...
    if (nrhs != 5) ERROR_MSG(INVALID_NUM_ARGS, "Expecting five arguments.");
    
	ASSERT_VECTOR_TYPE(RHS_ARG_2, COLUMN_VECTOR);
	
	ColumnVector q = NMArrayFromMxArray(RHS_ARG_2);
	double eps = mxGetScalar(RHS_ARG_3);
	double lambda_max = mxGetScalar(RHS_ARG_4);
	int ref = (int)mxGetScalar(RHS_ARG_5);
