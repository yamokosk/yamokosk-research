function Prob = plannerSolve(varargin)
% plannerSolve - Solves the vantage point planning problem for my thesis!
%
%   Prob = plannerSolve(Prob) attempts to solve the vantage point planning
%   problem for a problem domain already specified in Prob. See
%   plannerAssign for information on constructing such a Problem structure.
%
%   Prob = plannerSolve();

% Read inputs and create, if needed a Problem data structure
Prob = parseInputs(varargin{:});

  




% =========================================================================
% initialize(Prob)
%
%   Performs some simple initialization steps for the planner, including:
%       * Initialize output function
%       * Initialize solution data structure, if needed
% =========================================================================
function Prob = initialize(Prob)

% Notify the output function we are initializing
Prob.output_fcn(0, [], [], [], 'init', Prob);

% Create a new solution data structure, if necessary
if isempty(Prob.Solution) 
    G_new = graph();
    W0 = Prob.node_evaluate(Prob.x0, Prob);  % Evaluate the new node
    G_new = add_node(G_new, Prob.x0, W0);
else
    G_new = Prob.G; 
end


% =========================================================================
% parseInputs(...)
%
%   Performs some simple initialization steps for the planner, including:
%       * Initialize output function
%       * Initialize solution data structure, if needed
% =========================================================================
function Prob = parseInputs(varargin)
Prob = struct();
if (nargin == 1)
    Prob = varargin{1};
else
    Prob = plannerAssign(varargin{:});
end