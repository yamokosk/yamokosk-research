function Good_Solutions  = ika(Robot,T_f_tool)
%   IKA     Reverse position analysis for the PA-10-6
%       GOOD_SOLUTIONS = IKA(ROBOT,T_WCS_TOOL) performs 
%       reverse position analysis for the Mitsubishi PA-10 using Crane's 
%       method. The inputs are:
%
%       Robot - Robot object created using the Matlab Robot Toolbox
%
%       T_f_tool - Transformation matrix from the wcs to the tool frame
%
%       GOOD_SOLUTIONS will contain a maximum of four or minimum of zero
%       rows. Each row corresponds to a set of joint angles which are a
%       valid solution to the inverse kinematics problem. The algorthim
%       also takes into account the joint limits of the robot when
%       determining is a potential solution is valid.
%
...
% ***********************************************************************
% T_1_f = inv_tmatrix(Robot.base);
% T_base_wcs = eye(4);
% T_base_wcs(1:3,1:3) = T_wcs_base(1:3,1:3)';
% T_base_wcs(1:3,4) = -T_base_wcs(1:3,1:3) * T_wcs_base(1:3,4);

T_1_tool = inv_tmatrix(Robot.base) * T_f_tool;
[All_Solutions, GoodSolutions] = ikine_pa10(Robot,T_1_tool,verbose);

GoodIndex = find( GoodSolutions == 1);

Good_Solutions = All_Solutions(GoodIndex,:);
...
%a67 = L{6}.A;
alp67 = pi/2; % This comes from the t-matrix from 6 to tool
S6 = L{6}.D;
...
if ( s23 > 1e-9 ) error(['Error! Alpha_23 should be equal to 0 degrees.']); end
if ( c34 > 1e-9 ) error(['Error! Alpha_34 should be equal to 90 degrees.']); end
...
%--------------------------------------------------------------------------
[a71, S7, S1, alp71, th7, gam1] = close_loop(Robot,T_1_tool);
...
angles(:,1) = ph1;
angles(:,2) = th2 - L{2}.offset;
angles(:,3) = th3 - L{3}.offset;
angles(:,4) = th4;
angles(:,5) = th5;
angles(:,6) = th6;
...
%function [a71, S7, S1, alp71, th7, gam1] = close_loop(T_wcs_1,T_6_tool,T_wcs_tool,S1_Offset)
function [a71, S7, S1, alp71, th7, gam1] = close_loop(robot, T_1_tool)
...
%       Robot - Robot object created using the Matlab Robot Toolbox
%
%       T_1_tool - Transformation matrix from the robot's 1st frame to the 
%       tool frame
%
%   OUTPUTS:
...
T_1_6 = T_1_tool * inv_tmatrix(robot.tool);

S6_f = T_1_6(1:3,3);
a67_f = T_1_6(1:3,1);
P6_f = T_1_6(1:3,4);

%disp('[S6_f, a67_f, P6_f]')
%[S6_f, a67_f, P6_f]
