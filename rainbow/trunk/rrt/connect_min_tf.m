function [tf_opt,U_opt,X_opt,Xf,CPUTime]  = connect_min_tf(t0, x0, xf, odefun)
nstates = length(x0);
dim = nstates/2;
...
Prob = conAssign(@cost_min_tf, @cost_min_tf_grad, @cost_min_tf_hess, sparse(1,1), lb, ub, 'Robot2D', z0, ...
                            [], lb(1), ...
                            [], [], [], @con_min_tf, @con_min_tf_grad, [], ConsPattern, zeros(m,1), zeros(m,1));
...
zr = rand(n,1);
Prob.ConsPattern = con_min_tf_grad(zr,Prob);
