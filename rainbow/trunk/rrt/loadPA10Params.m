...
function InertiaData = mLoadInertiaData
% Use this function to calculate inertia data in modified DH notation
% Output: InertiaData = [Ixx, Iyy, Izz, Ixy, Iyz, Ixz]
data = GetInertiaData;
...
% above to the DH coordinate systems.
T(:,:,1) = eye(4);
T(:,:,2) = rotx(-pi/2) * rotz(-pi/2);
T(:,:,3) = rotx(pi/2) * rotz(pi);
T(:,:,4) = eye(4);
T(:,:,5) = rotx(pi/2);
T(:,:,6) = eye(4);
...
InertiaData = zeros(6,6);
for n = 1:6
    I = [data(n,1), data(n,2), data(n,3); 
         data(n,2), data(n,4), data(n,5); ...
         data(n,3), data(n,5), data(n,6)];
    I = T(1:3,1:3,n) * I * T(1:3,1:3,n)';
    InertiaData(n,:) = [I(1,1),I(2,2),I(3,3),I(1,2),I(2,3),I(1,3)]; 
end
