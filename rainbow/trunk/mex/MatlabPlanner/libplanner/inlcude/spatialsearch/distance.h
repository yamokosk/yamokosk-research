...
{
	typedef Configuration Query_item;

	MatlabDistance() : pdata(0) {};
	MatlabDistance(const mxArray* ptr) : pdata(ptr) {};

	double transformed_distance(const Query_item& p1, const Query_item& p2) const
	{
		...
		prhs[1] = const_cast<Query_item&>(p1).CreateMArray();
		prhs[2] = const_cast<Query_item&>(p2).CreateMArray();
	...
	double min_distance_to_rectangle(const Query_item& p, const CGAL::Kd_tree_rectangle<TreeTraits>& b) const
	{
		return distance_to_rectangle("min_distance_to_rectangle", p, b);	
	}

	template <class TreeTraits>
	double max_distance_to_rectangle(const Query_item& p, const CGAL::Kd_tree_rectangle<TreeTraits>& b) const
	{
		return distance_to_rectangle("max_distance_to_rectangle", p, b);	
	}

	template <class TreeTraits>
	double distance_to_rectangle(const char* funcName, const Query_item& p, const CGAL::Kd_tree_rectangle<TreeTraits>& b) const
	{
